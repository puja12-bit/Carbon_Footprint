        ...
        <Card className="bg-card md:col-span-2 lg:col-span-2 bg-gradient-to-br from-primary/10 to-transparent border-primary/20">
          ...
          <CardContent>
            {loadingStats ? <Skeleton className="h-8 w-full" /> : (
              <div className="grid grid-cols-3 gap-4" role="list" aria-label="Environmental impact equivalents">
                <div className="flex flex-col items-center justify-center p-2" role="listitem">
                  <TreePine className="h-6 w-6 text-green-500 mb-2" aria-hidden="true" />
                  <div className="text-lg font-bold" aria-label={`${stats?.treesEquivalent.toFixed(1)} trees planted equivalent`}>{stats?.treesEquivalent.toFixed(1)}</div>
                  <div className="text-[10px] text-muted-foreground text-center uppercase tracking-wider">Trees Planted</div>
                </div>
                <div className="flex flex-col items-center justify-center p-2 border-l border-border" role="listitem">
                  <BatteryCharging className="h-6 w-6 text-yellow-500 mb-2" aria-hidden="true" />
                  <div className="text-lg font-bold" aria-label={`${stats?.phonesCharged.toLocaleString()} phones charged equivalent`}>{stats?.phonesCharged.toLocaleString()}</div>
                ...
                <div className="flex flex-col items-center justify-center p-2 border-l border-border" role="listitem">
                  ...
                  <div className="text-lg font-bold" aria-label={`${stats?.homesEquivalent?.toFixed(2) || "0.00"} homes powered equivalent`}>{stats?.homesEquivalent?.toFixed(2) || "0.00"}</div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
            ...
            >
              {PERIODS.map((p) => (
                <button
                  ...
                  className={`px-3 py-1 text-xs rounded-md capitalize transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1 ${period === p ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
                >
                  {p}
                </button>
              ))}
              ...
              <figure aria-label={`Emission trends chart for the past ${period}`}>
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={history} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    />
                    <Area type="monotone" dataKey="co2Kg" stroke="hsl(var(--primary))" strokeWidth={2} fillOpacity={1} fill="url(#colorCo2)" name="CO₂ (kg)" />
                  </AreaChart>
                </ResponsiveContainer>
              </figure>
            )}
        ...
        <Card>
          ...
          <CardContent className="h-[300px] flex flex-col justify-center">
            ...
            <ul className="grid grid-cols-2 gap-2 mt-4 list-none p-0 m-0" aria-label="Category legend">
